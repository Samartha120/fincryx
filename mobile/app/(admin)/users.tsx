...
import { getAdminUsers, type User } from '@/src/api/adminUsersApi';
...
export default function AdminUsersScreen() {
  ...
  const [data, setData] = useState<{ items: User[]; total: number; limit: number }>({ items: [], total: 0, limit: 20 });
  ...
  const loadPage = useCallback(
    async (nextPage: number, opts?: { showLoader?: boolean }) => {
      ...
      try {
        const res = await getAdminUsers({ page: nextPage, limit: data.limit });
        ...
        ListEmptyComponent={
          loading ? null : (
            <View style={{ padding: 14, borderRadius: 12, borderWidth: 1, borderColor: '#eee' }}>
              <Text style={{ fontWeight: '700' }}>No users</Text>
              <Text style={{ color: '#666', marginTop: 6 }}>No users found for this page.</Text>
            </View>
          )
        }
        renderItem={({ item }) => (
          <View style={{ padding: 14, borderRadius: 12, borderWidth: 1, borderColor: '#eee' }}>
            ...
            <Text style={{ color: '#666' }}>{item.role.toUpperCase()} • OTP {item.isOtpVerified ? 'verified' : 'pending'}</Text>
          </View>
        )}
      />
